...
function ProductA(id:number,price:number,rating:number,brand:string,name:string,url:string) : Product {
...
class ProductB implements Product{
    ...
    constructor(id:number,price:number,rating:number,brand:string,name:string,url:string){
        this.id=id; this.price=price; this.rating=rating;
        this.brand=brand; this.name=name; this.url=url;
    }
...
function check(brand: string): void {
    //checking through update function
    update();
};

//cart update
function cartTotal(): void {
    const markedBoxes: NodeListOf<HTMLInputElement> = document.querySelectorAll('input[name="cart-selector"]:checked');

    let currentTotal: number = 0;

    markedBoxes.forEach((checkbox: Element) => {
        const inputElement = checkbox as HTMLInputElement;
        const price: number = parseFloat(inputElement.dataset.price as string);
        if (!isNaN(price)) {
            currentTotal += price;
        }
    });

    const cartCost: HTMLElement | null = document.getElementById('cart-cost');
    if (cartCost) {
...
function toggleBorder(): void {
    ...
    const checkbox: HTMLInputElement | null = document.getElementById('border') as HTMLInputElement;
...
function toggleImages(): void {
    const showImages: boolean = (document.getElementById('images') as HTMLInputElement).checked;
...
function renderTheeProducts(products: Product[]): void {
    const listContainer: HTMLElement | null = document.getElementById('main');
    if (!listContainer) return;

    //current cart
    const currentCart: Record<string, boolean> = {};

    document.querySelectorAll('input[name="cart-selector"]').forEach((cb: Element) => {
        const inputElement = cb as HTMLInputElement;
        const itemIdentifier: string | undefined = inputElement.dataset.name;
        if (itemIdentifier) {
    ...
    products.forEach((product: Product) => {
        ...
        const imageClass: string = showImages ? '' : 'hidden-image';

        const itemIdentifier: string = `${product.brand}: ${product.name}`;
        const isChecked: boolean = currentCart[itemIdentifier] != undefined ? currentCart[itemIdentifier] : false;
        const checkedAttr: string = isChecked ? 'checked' : '';
...
function update(): void {
    ...
    //price filter
    const maxPrice: number = parseFloat((document.getElementById('price') as HTMLSelectElement).value);
    if (maxPrice < 10000) {
        filteredProducts = filteredProducts.filter((product: Product) => product.price <= maxPrice);    
    };

    //rating filter
    const minRate: number = parseFloat((document.getElementById('rating') as HTMLSelectElement).value);
    filteredProducts = filteredProducts.filter((product: Product) => product.rating >= minRate);

    //brand filter
    const brands: string[] = ["Adidas","Crocs","Nike","Puma","Under Armour"];
    const selectBrand: string[] = [];
    ...
    brands.forEach((brand: string) => {
        ...
        const brandCheckbox = document.getElementById(realId) as HTMLInputElement;
    ...
    if (selectBrand.length > 0) {
        filteredProducts = filteredProducts.filter((product: Product) => selectBrand.includes(product.brand));
    };

    const sortByElement = document.getElementById('sort-by') as HTMLSelectElement;
    const sortBy: string = sortByElement ? sortByElement.value : 'default';
...
function runLabProblems(): void {
//1) See Nike brand shoes (filter by brand)(All brands?)

    function brandFilter(products:Product[]): Map<string, Product[]> {
...
function initApp(): void {
    ...
    (window as any).toggleBorder = toggleBorder;
    (window as any).toggleImages = toggleImages;
