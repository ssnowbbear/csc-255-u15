...
function cut2(carat, small) {
    ...
    if (carat % 3 == 0) {
        ...
        return cut2(Math.floor(3 * carat / 5), small) + cut2(Math.floor(2 * carat / 5), small);
}
console.log(cut2(10, 2)); //4
//hard problem
var prices = [0, 1, 5, 8, 9, 10, 17, 17, 20];
var memo = {};
function solve(carat) {
    var _a;
    if (carat in memo)
        return memo[carat];
    if (carat <= 1)
        return prices[carat]; //base case
    var best = (_a = prices[carat]) !== null && _a !== void 0 ? _a : 0;
    for (var i = 1, j = carat - 1; i <= j; i++, j--) {
